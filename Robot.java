...
class ParkingSpaceSmartArray {
    ...
    public Integer getAvailableCapacityByTime(Calendar entranceTime, Calendar departureTime) {
        ...
        for (Reservation reservation : reservations) {
            ...
            if (!(vehicleDepartureTime.before(entranceTime) || reservation.getArrivalDate().after(departureTime)))
                numOfUnavailablePS++;
        }
    ...
    public Reservation UpdateReservation (Vehicle vehicle){
        ...
        if (reservations.size() > 0) {
            Collections.sort(reservations, Comparator.comparing(Reservation::getArrivalDate));
