...
public class Robot implements Serializable {
    ...
    public Robot (ParkingLot parkingLot) {
        parkingSpaceSmartArray = new ParkingSpaceSmartArray(3,5,3);
...
class ParkingSpaceSmartArray {
    ...
    private ParkingLot parkingLot;
    //BUILD CONSTRUCTOR
    public ParkingSpaceSmartArray(Integer x, Integer y, Integer z) { //get dims
    ...
    public boolean insert (Vehicle vehicle, Reservation reservation) {
        ...
        //loop: run on the array. stop condition: (available||out over range||find place between cars (exit time))
    ...
    //set a condintion to specific parking place
    ...
    public boolean setConditionToPlace(int row, int col, int floor, ParkingSpaceCondition condition, Reservation reservation){
        ...
        if (array[row][col][floor].getCondition()==ParkingSpaceCondition.AVAILABLE){
            array[row][col][floor].setCondition(condition);
            return true;
        }else if (array[row][col][floor].getCondition()==ParkingSpaceCondition.OCCUPIED){
            //save car
